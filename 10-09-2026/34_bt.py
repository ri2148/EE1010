...
if abs(b1) > 1e-9:
    ...
    plt.axvline(x=c1/a1, label=f'Line 1: x = {c1/a1:.2f}', color='blue', linewidth=2.5)

if abs(b2) > 1e-9:
    y_vals2 = (c2 - a2 * x_vals) / b2
    plt.plot(x_vals, y_vals2, label=f'Line 2: {a2:.0f}x + {b2:.0f}y = {c2:.1f}', color='red', linestyle='--', linewidth=2)
else:
    plt.axvline(x=c2/a2, label=f'Line 2: x = {c2/a2:.2f}', color='red', linestyle='--', linewidth=2)
